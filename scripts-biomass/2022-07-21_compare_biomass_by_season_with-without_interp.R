# RRR
# re-make figure 3 without using data from the linear interpolation- Paul was concerned about using modeled data
...
biomass <- readRDS("robin-data/2022-06-23_biomass_metrics/biomass_metrics_by_season-measured_values.rds")
names(biomass)
names(biomass$mean)
head(biomass$mean$spring)

measured <- readRDS("robin-data/2022-06-23_biomass_metrics/biomass_metrics_by_season-individual_measurements")
names(measured)
names(measured$mean)
head(measured$mean$spring)

plot.folder <- "plots/2022-07-08_biomass_by_season_boxplots/no-interp/"
...
pdf(file = file.path(plot.folder,"Seasonal_Biomass_Change.pdf"), width = 7, height = 3)
...
pdf(file = file.path(plot.folder,"Seasonal_Biomass_Change-with_jitter.pdf"), width = 7, height = 3)
