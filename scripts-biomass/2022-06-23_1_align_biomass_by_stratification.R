...
strat <- read.csv(file = "output/stratification_start.csv") # all in year 2000 dates FYI

phyto <- readRDS("robin-data/2022-06-09_annual_patterns/phyto_split_by_year.rds")

output.folder <- "robin-data/2022-06-23_biomass_aligned_by_strat_onset/"
...
saveRDS(object = phyto.interps.all, file = paste0(output.folder,"interp.tables.all.rds"))
saveRDS(object = phyto.SWF.comparison, file = paste0(output.folder, "interp.tables.pre-post.rds"))
